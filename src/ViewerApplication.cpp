...
int ViewerApplication::run()
{
  ...
  if (m_hasUserCamera) {
    ...
    const glm::vec3 center = glm::vec3((bboxMin.x+bboxMax.x)/2,(bboxMin.y+bboxMax.y)/2,(bboxMin.z+bboxMax.z)/2);
    
    std::cout<<"UP :"<<m_upVector<<std::endl;
    const auto up = ( m_upVector=="y" ? glm::vec3(0,1,0) : glm::vec3(0,0,1) );
    const auto eye = ( diag.z<=0 ? center + 2.f * glm::cross(diag, up) : center+diag);
  ...
  std::vector<VaoRange> meshToVtxArrays;
  const auto vertexArrayObjects =
      createVertexArrayObjects(model, bufferObjects, meshToVtxArrays);
    ...
    const std::function<void(int, const glm::mat4 &)> drawNode =
        [&](int nodeIdx, const glm::mat4 &parentMatrix) {
          ...
          if (node.mesh >= 0) {
            ...
            const auto &vaoRange = meshToVtxArrays[node.mesh];
...
std::vector<GLuint> ViewerApplication::createVertexArrayObjects( const tinygltf::Model &model, const std::vector<GLuint> &bufferObjects, std::vector<VaoRange> & meshToVtxArrays){
  std::vector<GLuint> vertexArrayObjects;

  // For each mesh of model we keep its range of VAOs
  meshToVtxArrays.resize(model.meshes.size());
  ...
  for(int meshIdx=0; meshIdx<model.meshes.size(); ++meshIdx){
    ...
    // Save the value
    auto &vaoRange = meshToVtxArrays[meshIdx];
